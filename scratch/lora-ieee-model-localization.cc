...
int nGateways = 6; 
 ...
 double simulationTimeSeconds = 600; //!< Scenario duration (s) in simulated time 600 
 uint32_t beaconRoundCounter = 0; //!< Counter for beacon rounds
...
{
    // IEEE 802.11 Sub 1GHz Urban Macro Path Loss Model (Model 1)
    double correctionFactor = 21.0 * std::log10(868.0 / 900.0); // ≈ -0.34 dB
    double pathLossAtD0 = 8.0 + correctionFactor; // ≈ 7.66 dB at d0 = 1m
    double pathLossExponentFactor = 37.6;         // Fixed from model (not 10*n)

    // Estimate Path Loss
    double pathLossDb = txPowerDbm - rxPowerDbmWithNoise;

    // Calculate distance using inverted model: PL = PL(d0) + B * log10(d)
    double logDistance = (pathLossDb - pathLossAtD0) / pathLossExponentFactor;
    double estimatedDistance = std::pow(10.0, logDistance);
...
// The function takes the following parameters:
 ...
 void SendBeaconFromGateway(uint32_t gwIndex,
    ...
    double sensitivityThresholdDbm = -125.0;   // Sensitivity threshold in dBm (e.g., -137 dBm for SX1276 at BW 125kHz, CR 4/5))
                                                 // Sensitivity threshold in dBm: SF7, BW 125 kHz: around -121 dBm to -125 dBm (e.g., Semtech datasheets often quote -123 dBm)         
    ...
    // Calculate received power at the end device
    double rxPowerDbm = channel->GetRxPower(txPowerDbm, gwMob, edMob); // This is signal power after path loss
    double noiseDb = noise->GetValue(); // This 'noise' is passed from SendBeaconFromAllGateways
    rxPowerDbm += noiseDb; // Add noise to the received power
    ...
    // Timeline log for visualization
 ...
 void SendBeaconFromAllGateways(ns3::NodeContainer gateways,
    ...
    noise->SetAttribute("Variance", DoubleValue(0.0));  // Adjust variance as needed: initial = std: 4.0, let tune it to 1.0, or 0.5 etc...

    Time now = Simulator::Now();
    beaconRoundCounter++; // Increment the beacon round counter
...
void ComputeAverageDownlinkPDR(std::string pdrFilename, uint32_t nGateways)
{
    ...
    if (!file.is_open())
    {
        std::cerr << "Error: Failed to open " << pdrFilename << std::endl;
        return;
    }
    ...
    std::string line;
    while (std::getline(file, line)) // Reads one full line, e.g., "60.0s, GW0, 95.0%"
    {
        std::istringstream iss(line);
        std::string timeStr, gwIdentifierStr, pdrPercentStr;

        // Parse the line based on commas
        if (std::getline(iss, timeStr, ',') &&       // First part (time)
            std::getline(iss, gwIdentifierStr, ',') && // Second part (GW identifier)
            std::getline(iss, pdrPercentStr))         // Third part (PDR percentage, rest of the line)
        {
            // Trim leading/trailing whitespace from gwIdentifierStr and pdrPercentStr
            gwIdentifierStr.erase(0, gwIdentifierStr.find_first_not_of(" \t\n\r\f\v"));
            gwIdentifierStr.erase(gwIdentifierStr.find_last_not_of(" \t\n\r\f\v") + 1);

            pdrPercentStr.erase(0, pdrPercentStr.find_first_not_of(" \t\n\r\f\v"));
            pdrPercentStr.erase(pdrPercentStr.find_last_not_of(" \t\n\r\f\v") + 1);

            // Extract GW index from gwIdentifierStr (e.g., "GW0" -> 0)
            if (gwIdentifierStr.rfind("GW", 0) == 0) { // Check if it starts with "GW"
                try {
                    std::string gwIndexStr = gwIdentifierStr.substr(2); // Get the number part
                    uint32_t currentGwIndex = std::stoul(gwIndexStr);

                    if (currentGwIndex < nGateways) {
                        // Remove the '%' sign from pdrPercentStr
                        pdrPercentStr.erase(std::remove(pdrPercentStr.begin(), pdrPercentStr.end(), '%'), pdrPercentStr.end());
                        double pdrValue = std::stod(pdrPercentStr);
                        totalPdr[currentGwIndex] += pdrValue;
                        count[currentGwIndex]++;
                    } else {
                        // NS_LOG_WARN("Parsed GW index " << currentGwIndex << " out of bounds for nGateways=" << nGateways << " in line: " << line);
                    }
                } catch (const std::invalid_argument& ia) {
                    // NS_LOG_WARN("Invalid argument for GW index conversion in line: " << line << " - " << ia.what());
                } catch (const std::out_of_range& oor) {
                    // NS_LOG_WARN("Out of range for GW index conversion in line: " << line << " - " << oor.what());
                }
            } else {
                 // NS_LOG_WARN("Could not parse GW identifier in line: " << line);
            }
        } else {
            // NS_LOG_WARN("Could not parse line format: " << line);
        }
    }
    ...
    if (!outFile.is_open())
    {
        std::cerr << "Failed to open " << avgPdrFileNameStream.str() << " for writing." << std::endl;
        return;
    }
...
void VerifyOutputFiles(const std::string& folder, int nDevices, int nGateways, double radiusMeters)
{
 ...
 main(int argc, char* argv[])
 {
    ...
    roundLocLog << "Round,Time(s),ED_ID,ActualX,ActualY,EstimatedX,EstimatedY,Error(m)\n";
    roundLocLog.close();
    ...
    //  LogComponentEnable("ClassAEndDeviceLorawanMac", LOG_LEVEL_ALL);
     ...
     loss->SetReference(1, 7.7);   // check literature for reference distance and loss at reference distance in LoRa
    ...
    allocator->Add(Vector(-700.0, 700.0, 10.0));  // GW0 (Adjusted for broader coverage)
    allocator->Add(Vector(700.0, 700.0, 10.0));   // GW1
    allocator->Add(Vector(-700.0, -700.0, 10.0)); // GW2
    allocator->Add(Vector(700.0, -700.0, 10.0));  // GW3
    allocator->Add(Vector(0.0, 0.0, 10.0));       // GW4 (Central)
    allocator->Add(Vector(0.0, 950.0, 10.0));     // GW5 (North, near edge of 1000m radius)
    ...
    // display the total number of beacon rounds
    uint32_t totalRounds = static_cast<uint32_t>(simulationTimeSeconds / beaconInterval.GetSeconds());
    std::cout << "Total possible beacon rounds: " << totalRounds << std::endl;
