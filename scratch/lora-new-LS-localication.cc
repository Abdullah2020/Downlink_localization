...
double EstimateDistanceFromRSSI(double txPowerDbm, double rxPowerDbmWithNoise)
{
    ...
    double d0 = 1.0;                         // Reference distance in meters (matches loss->SetReference(1, ...))
    double pathLossAtD0 = 7.7;               // PL(d0) in dB (matches loss->SetReference(..., 7.7))
    double pathLossExponent = 3.76;          // Environment dependent (matches loss->SetPathLossExponent(3.76))

    // Calculate Path Loss (PL) based on received power
    // RxPower = TxPower - PL => PL = TxPower - RxPower
    double pathLossDb = txPowerDbm - rxPowerDbmWithNoise;

    // Now, invert the path loss formula: PL(d) = PL(d0) + 10*n*log10(d/d0)
    // pathLossDb - pathLossAtD0 = 10 * pathLossExponent * log10(distance / d0)
    // (pathLossDb - pathLossAtD0) / (10.0 * pathLossExponent) = log10(distance / d0)
    // distance / d0 = 10^((pathLossDb - pathLossAtD0) / (10.0 * pathLossExponent))
    // distance = d0 * 10^((pathLossDb - pathLossAtD0) / (10.0 * pathLossExponent))

    // Handle cases where calculated path loss is less than path loss at reference distance
    // This can happen due to noise making rxPowerDbmWithNoise > (txPowerDbm - pathLossAtD0)
    // or if the actual distance is less than d0.
    // Clamping distance to a small positive value or d0 can be a pragmatic approach.
    if (pathLossDb < pathLossAtD0) {
         // This implies distance < d0 or very favorable noise.
         // Option 1: Estimate as d0 (or a bit less) if you expect devices not to be closer.
         // Option 2: Calculate directly. If the term inside pow becomes very negative, distance -> 0.
         // For simplicity here, let's allow it, but be aware of potential for very small distances.
    }

    double exponent_val = (pathLossDb - pathLossAtD0) / (10.0 * pathLossExponent);
    double estimatedDistance = d0 * std::pow(10.0, exponent_val);

    // It's good practice to ensure distance is not zero or negative if the formula could lead to it.
    if (estimatedDistance <= 0) {
        estimatedDistance = 0.1; // Or some other small positive default
    }
...
// The function takes the following parameters:
 ...
 void SendBeaconFromAllGateways(ns3::NodeContainer gateways,
    ...
    noise->SetAttribute("Variance", DoubleValue(1.0));  // Adjust variance as needed: initial = std: 4.0, let tune it to 1.0, or 0.5 etc...
